...
*/
      ...
      if( dataType_=="PAT" )
      {
         ...
         if(verbosity_>1) std::cout << " currentJECSet()   = " << patJet->currentJECSet()   << std::endl;
         if(verbosity_>1) std::cout << " currentJECLevel() = " << patJet->currentJECLevel() << std::endl;
         if(verbosity_>1) std::cout << "=========================================" << std::endl;
         if(verbosity_>1) std::cout << " available JEC sets:                     " << std::endl;
         
         const std::vector<std::string> jecSets = patJet->availableJECSets();
         for ( unsigned int k=0; k<jecSets.size(); ++k)
         {
            if(verbosity_>1) std::cout << jecSets[k] << endl;
            const std::vector<std::string> jecLevels = patJet->availableJECLevels(jecSets[k]);
            for ( unsigned int l=0; l<jecLevels.size(); ++l)
            {
               if(verbosity_>1) std::cout << jecLevels[l] << endl;
